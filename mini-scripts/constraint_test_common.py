...
class testwatchelement_one_undefzero:
    def printtable(self, domains): 
        out=[]
        # given a list of lists for the domains, generate the table for gacelement 
        numvars=len(domains)
    ...
    def runtest(self, options=dict()):
        return runtestgeneral("watchelement_one_undefzero", False, options, [4,1,1], ["smallnum", "num", "num"], self, not options['reify'])
...
class testhamming:
    ...
    def runtest(self, options=dict()):
        return runtestgeneral("hamming", False, options, [4,4,1], ["smallnum", "smallnum", "const"], self, not options['reify'])
