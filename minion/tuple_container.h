...
public:
  ...
  DomainInt *getPointer() { return tuple_data; }

  /// Get number of tuples.
  DomainInt size() const { return number_of_tuples; }

  // Get size of a particular tuple.
  DomainInt tuple_size() const { return tuple_length; }
  ...
  TupleList(const vector<vector<DomainInt>> &tuple_list)
      : litlists(NULL), nightingale(NULL), triearray(NULL), regin(NULL), egg(NULL),
        tuples_locked(false), hash_code(0) {
  ...
  const DomainInt *operator[](SysInt pos) const { return get_tupleptr(pos); }
  ...
  void setName(string name) { tuple_name = name; }

  string getName() const { return tuple_name; }
...
public:
  ...
  TupleList *getTupleList(DomainInt num) { return Internal_TupleList[checked_cast<SysInt>(num)]; }

  SysInt size() { return Internal_TupleList.size(); }
};
...
public:
  ...
  vector<set<DomainInt>> getInitialDomains() { return initial_domains; }
  ...
  SysInt size() const { return short_tuples.size(); }

  vector<vector<pair<SysInt, DomainInt>>> const *tuplePtr() const { return &short_tuples; }
  ...
  LiteralSpecificLists(TupleList *_tuples) : tuples(_tuples) {
    ...
    for (UnsignedSysInt i = 0; i < tuples->dom_size.size(); ++i) {
      for (DomainInt j = tuples->dom_smallest[i];
           j <= tuples->dom_smallest[i] + tuples->dom_size[i]; ++j) {
