...
public:
  ...
  void lock(SysInt size, DomainInt min_domain_val, DomainInt max_domain_val)
  {
    ...
    if(only_bounds)
      dynamic_triggers = getMemory(stateObj).allocate(size * sizeof(DynamicTrigger) * 4);
    else
      dynamic_triggers = getMemory(stateObj).allocate(size * sizeof(DynamicTrigger) * (4 + vars_domain_size));
#else
    if(only_bounds)
