...
/// TupleGrabNargs<std::tuple<Arg1,...,Argn, Args...>>::type = std::tuple<Arg1,...,Argn>
template<typename Tuple, int n>
struct TupleGrabNargs;

/// Given Tuple = std::tuple<Arg1,...,Argn, Args...>,
/// TupleGrabNargs<std::tuple<Arg1,...,Argn, Args...>>::type = std::tuple<Args...>
template<typename Tuple, int n>
...
struct TemplateStackConstructor
{
	...
	struct Type
	{
		typedef typename JoinFromTuple<T,
				   typename removePointerFromTupleArgs<
	                 typename TupleGrabNargs<std::tuple<Args...>,ArgCount>::type
	               >::type
	            >::type type;
	};

    template<typename... Args>
    typename Type<Args...>::type
		...
		typedef    typename JoinFromTuple<T,
	    		     typename TupleJoin<
	    		       typename removePointerFromTupleArgs<
	                     typename TupleGrabNargs<std::tuple<Args...>, ArgCount>::type
	                   >::type,
	                   typename TupleDropNargs<std::tuple<Args...>, ArgCount>::type
	                 >::type
	               >::type type;
	};
...
private:
    ...
    typename Constructor::template Type<Args..., CArgs...>::type
    impl(IntTuple<Val...>, const CArgs&... cargs)
    {
        Constructor c;
...
public:
    ...
    typename Constructor::template Type<Args..., CArgs...>::type
    operator()(const CArgs&... cargs)
    {
        return impl(typename MakeIntList<SizeOf<Args...>::size>::type(),
                    cargs...);
    }
...
// using ObjectResult = decltype(instance<typename removeAllPointers<Factory>::type>()(instance<Args>()...));
...
// There is no way of defining both template<typename T = int> struct S{}; and struct P; in a consistent manner.
// As we have to write S<> s; but P p; NOARG_HACK(S) s; and NOARG_HACK(P) p; both work.
