...
public:
    ...
    // The reason these are explicit is because constructing an 'ImmutableString' is fairly
    // expensive, so we want to know when we do it.

    explicit ImmutableString(const std::string& s) : ptr(ImmutableStringCache::getInstance().cachedString(s))
    { }

    explicit ImmutableString(const char* s) : ptr(ImmutableStringCache::getInstance().cachedString(s))
    { }
