...
// There is no restriction what values (which are in the type) can be set. 
//
...
public:
  ...
  DomainInt get(DomainInt index) const
  {
    return array(index);   
  }
...
#ifdef DEBUG
    cout << "initialising TrailedValues with value of size= " << size << endl;
#endif
...
void initialise_value(DomainInt index, const value_type& newval) 
  {
    array(index) = newval;
  }
...
void print_state()
{
  cout << "printing state of TrailedValues: " ;
