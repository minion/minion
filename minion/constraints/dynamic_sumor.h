...
struct VecCountDynamic : public AbstractConstraint {
  virtual string constraint_name() { return Operator::constraint_name(); }
  ...
  VecCountDynamic(const VarArray1 &_array1, const VarArray2 &_array2, DomainInt _hamming_distance)
      : var_array1(_array1), var_array2(_array2), num_to_watch(_hamming_distance + 1),
        hamming_distance(_hamming_distance), propagate_mode(false), index_to_not_propagate(-1) {
    if (num_to_watch <= 1)
      num_to_watch = 0;
