...
public:
  ...
  *  as returned by get_vars.
  ...
  virtual void setup()
  {
    ...
    D_ASSERT(trigs >= 0);
    setup_dynamic_triggers(getMemory(stateObj).allocate((sizeof(DynamicTrigger) * trigs)));
...
public:
  ...
  virtual void setup()
  {
    ...
    void* trigMem = getMemory(stateObj).allocate(sizeof(DynamicTrigger) * all_trigs);
