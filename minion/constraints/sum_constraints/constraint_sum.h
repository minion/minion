...
AbstractConstraint*
greaterEqualSumConstraint(const vector<VarArray>& _var_array, const VarSum _var_sum)
{
...
template<typename VarArray,  typename VarSum>
AbstractConstraint*
lessEqualSumConstraint(const vector<VarArray>& _var_array, const VarSum _var_sum)
{
  if(_var_array.size() == 2)
  {
    std::array<VarArray, 2> v_array;
...
template<typename VarArray,  typename VarSum>
AbstractConstraint*
BuildCT_LEQSUM(const VarArray& _var_array, const vector<VarSum>& _var_sum, ConstraintBlob&)
{ 
  if(_var_array.size() == 2)
  {
...
BuildCT_LEQSUM(const vector<BoolVarRef>& var_array, const vector<ConstantVar>& var_sum, ConstraintBlob&)
{ 
...
template<typename T1, typename T2>
AbstractConstraint*
BuildCT_WEIGHTLEQSUM(vector<T1> vec, const vector<T2>& t2, ConstraintBlob& b)
{
  vector<DomainInt> scale = b.constants[0];
  // Preprocess to remove any multiplications by 0, both for efficency
  // and correctness
