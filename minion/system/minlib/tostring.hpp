...
T fromstring(const std::string& str)
{
  ...
  if(!iss.eof()) throw std::runtime_error("Failed conversion of '" + str + "'");
  return obj;
}
...
std::string
tostring(const T1& t1, const T2& t2)
{
  std::ostringstream streamOut;
  streamOut << t1 << " " << t2;
  return streamOut.str();
...
is_printable(const T& t);

std::false_type
is_printable(AnyGrab);

template<typename T>
std::string
try_tostring_impl(const T& t, std::true_type)
{ return tostring(t); }

template<typename T>
std::string
try_tostring_impl(const T& t, std::false_type)
{ return std::string("<no printer for ") + typeid(t).name() + ">"; }

template<typename T>
std::string
try_tostring(const T& t)
{
