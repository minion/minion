...
class MiniStream    {
    ...
    bool get_string(string &s){
        ...
        while (isalnum(peek()) || peek() == '_') {   //copy valid characters
            ...
            pos++;
            incr();
            if (pos >= 999) { p_error("Identifier too long:"); }}
    ...
    void p_error(const string error){ p_error(error, charnum);  }
    ...
    void add_var(string name, vector<int> bounds, std::map<vector<int>,int> storage){
        ...
        for (auto i : storage) { temp.add(i.first, i.second); }
...
public:
    ...
    MiniStream(string fname, simmap &cm){
        ...
        s = string((std::istreambuf_iterator<char>(file)),
        ...
        while(!eof()){
            ...
            if (var_name != "letting") {                            //letting
                p_error("Expected 'letting' here:",
                        charnum - var_name.length()); }
            ...
            if (is_sign('[')) {                                     //array
                int depth = 0;
                int c_lvl = 0;
                vector<int> array_pointer;  array_pointer.push_back(0);
                vector<int> array_max;      array_max.push_back(1);
