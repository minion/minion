...
public:
  ...
  bool has(const T &name) const { return props.count(name); }

  U &get(const T &name) { return mapget(props, name); }

  const U &get(const T &name) const { return mapget(props, name); }
  ...
  friend bool operator==(const SimpleMap &l, const SimpleMap &r) { return l.props == r.props; }

  friend bool operator<(const SimpleMap &l, const SimpleMap &r) { return l.props < r.props; }

  friend std::ostream &operator<<(std::ostream &os, const SimpleMap &sm) { return os << sm.props; }
  ...
  unsigned size() const { return props.size(); }
};
#endif
