...
struct TupleGrabNargs<std::tuple<Arg, Args...>, i> {
  typedef typename TupleAddArgFront<typename TupleGrabNargs<std::tuple<Args...>, i - 1>::type,
                                    Arg>::type type;
};
...
// This overload doesn't look that useful, we need it because the compiler can't
// choose between the
// above two options
template <typename Arg, typename... Args>
