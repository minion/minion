...
int main (int argc, char * const argv[]) {
  ...
  vector<vector<LRangeVarRef> > vars(height);
  for(int i=0;i<height;i++)
    for(int j=0;j<width;j++)
      vars[i].push_back(rangevar_container.get_new_var(0,1));
  
  array<LRangeVarRef,height> row;
  array<LRangeVarRef,width> col;
  for(int i=0;i<height;i++)
  {
    for(int j=0;j<width;j++)
      col[j]=vars[i][j];
  ...
  vector<vector<LRangeVarRef> > scalar((height*(height-1))/2);
  for(unsigned int i=0;i<scalar.size();i++)
    for(int j=0;j<width;j++)
      scalar[i].push_back(rangevar_container.get_new_var(0,1));
  int count = 0;
  for(int i=0;i<height;i++)
  {
