...
struct SparseBoundVarContainer {
  ...
  vector<DomainInt> get_raw_domain(DomainInt i) { return this->domains[checked_cast<SysInt>(i)]; }
...
#ifdef WDEG
  DomainInt getBaseWdeg(const SparseBoundVarRef_internal<BoundType> &b) { return wdegs[b.var_num]; }

  void incWdeg(const SparseBoundVarRef_internal<BoundType> &b) { wdegs[b.var_num]++; }
#endif

  void addDynamicTrigger(AbstractConstraint *ac, SparseBoundVarRef_internal<BoundType> b,
