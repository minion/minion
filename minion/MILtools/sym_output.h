...
build_graph(std::vector<std::set<SysInt> > graph, const
std::vector<std::set<SysInt> >& partition);

SysInt
repartition(const std::vector<std::set<SysInt> >& graph, std::vector<SysInt>
partition_num)
{
  std::vector<std::multiset<SysInt> > partition_loop(graph.size());
  for(SysInt i = 0; i < (SysInt)graph.size(); ++i)
    for(set<SysInt>::const_iterator it = graph[i].begin(); it != graph[i].end();
++it)
    {
  ...
  std::set<std::multiset<SysInt> > partition_set(partition_loop.begin(),
partition_loop.end());
  std::vector<std::multiset<SysInt> > partition_vec(partition_set.begin(),
partition_set.end());

  for(SysInt i = 0; i < (SysInt)graph.size(); ++i)
  {
    partition_num[i] = find(partition_vec.begin(), partition_vec.end(),
partition_loop[i]) - partition_vec.begin();
  }
...
double
partition_graph(const std::tuple<SysInt,vector<set<SysInt> >,vector<set<SysInt>
> >& graph_tuple)
{
  ...
  {
    for(std::set<SysInt>::iterator it = partition[i].begin(); it !=
partition[i].end(); ++it)
      partition_num[*it] = i;
  }
  ...
  return (double)(diff_count) / (double)(partition_num.size() *
partition_num.size());
  ...
  void output_graph()
   {
     ...
     {
       cout << it->first << " : ";
       for(set<string>::iterator it2 = it->second.begin(); it2 !=
it->second.end(); it2++)
         cout << *it2 << " ";
       cout << endl;
     }
...
build_graph_info(CSPInstance& csp, bool print_names = true)
   {
     ...
     for(map<string, set<string> >::iterator it = var_vertex_colour.begin(); it
!= var_vertex_colour.end(); ++it)
       var_vertex_count += it->second.size();

     for(map<string, set<string> >::iterator it = aux_vertex_colour.begin(); it
!= aux_vertex_colour.end(); ++it)
...
it->second.end(); it2++)
     ...
     for(set<pair<string, string> >::iterator it = graph.begin(); it !=
graph.end(); ++it)
     {
       //cout << it->first << ":" << it->second << endl;
...
struct GraphBuilder
{
  ...
  GraphBuilder(CSPInstance& _csp) : csp(_csp)
  {
    ...
    if(csp.sym_order.empty())
      D_FATAL_ERROR("Symmetry detection doesn't work with input formats 1 and 2.
Upgrade!");
...
//      return aux_v;
  ...
  string colour_no_symmetry(const ConstraintBlob& b, string name)
  {
    ...
    for(SysInt i = 0; i < (SysInt)b.vars.size(); ++i)
      for(SysInt j = 0; j < (SysInt)b.vars[i].size(); ++j)
      {
        string vij = g.new_vertex(name + "_CHILD_" + tostring(i) + ";" +
tostring(j));
        add_edge(v, vij);
...
second array.
  ...
  string colour_symmetric_parent_constraint(const ConstraintBlob& b, string
name)
  {
  ...
  string colour_constraint(const ConstraintBlob& b)
  {
    switch(b.constraint->type)
    {
      ...
      case CT_CHECK_ASSIGN : return
colour_constraint(b.internal_constraints[0]);
      ...
      case CT_WATCHED_ELEMENT_UNDEFZERO: return colour_element(b,
"ELEMENT_UNDEFZERO");
      ...
      case CT_WATCHED_VEC_OR_LESS: return colour_symmetric_indexes(b,
"VEC_OR_LESS");


      case CT_WATCHED_HAMMING: return colour_array_swap_each_index(b,
"HAMMING");
      ...
      case CT_WATCHED_NOT_INRANGE: return colour_litlist(b,
"WATCHED_NOT_INRANGE");
      ...
      case CT_WATCHED_NEW_OR: return colour_symmetric_parent_constraint(b,
"OR");


      case CT_WATCHED_NEW_AND: return colour_symmetric_parent_constraint(b,
"AND");
...
struct InstanceStats
{
  ...
  void classifyConstraint(ConstraintBlob i, SysInt* alldiff, SysInt* sums,
SysInt* or_atleastk, SysInt* ternary, SysInt* binary, SysInt* table, SysInt*
reify, SysInt* lex, SysInt* unary, SysInt* nullary, SysInt* element, SysInt*
minmax, SysInt* occurrence, vector<double>* alldiffdomovervars, VarContainer& v)
{
        ...
        case CT_GACALLDIFF:
            {
                ...
                for(SysInt j = 0; j < (SysInt)i.vars.size(); j++) {
                    for(SysInt k = 0; k < (SysInt)i.vars[j].size(); k++) {
                        ...
                        lower = lower > bounds.lower_bound ? bounds.lower_bound
: lower;
                        upper = upper < bounds.upper_bound ? bounds.upper_bound
: upper;
                    }
                }
                alldiffdomovervars->push_back((double)(checked_cast<SysInt>(
upper -  lower + 1)) / (double) num);
            }
            break;
        case CT_GEQSUM:
        case CT_LEQSUM:
        ...
        case CT_REIFYIMPLY:
            (*reify)++;
            for(vector<ConstraintBlob>::iterator
j=i.internal_constraints.begin(); j!=i.internal_constraints.end(); ++j)
            {
                classifyConstraint(*j, alldiff, sums, or_atleastk, ternary,
binary, table, reify, lex, unary, nullary, element, minmax, occurrence,
alldiffdomovervars, v);
            }
            break;
        ...
        case CT_GCCWEAK:
            ...
            break;
        default:
            cerr << "Stats: Uncategorised constraint:" << i.constraint->name
<<endl;
  ...
  void output_stats()
  {
      ...
      SysInt
varcount=v.BOOLs+v.bound.size()+v.sparse_bound.size()+v.discrete.size();
      ...
      {
          SysInt dom_size =
checked_cast<SysInt>(v.bound[i].second.upper_bound-v.bound[i].second.lower_bound+1);
      ...
      {
          SysInt dom_size =
checked_cast<SysInt>(v.discrete[i].second.upper_bound-v.discrete[i].second.lower_bound+1);
          domsizes.push_back(dom_size);
      ...
      SysInt totaldom=checked_cast<SysInt>(std::accumulate(domsizes.begin(),
domsizes.end(), (DomainInt)0));
      cout << s << "dom_mean:" << ((double)totaldom)/(double) domsizes.size() <<
endl;

      SysInt num2s= std::count(domsizes.begin(), domsizes.end(), (DomainInt)2);
      cout << s << "dom_not2_2_ratio:" << ((double) (varcount-num2s)
)/(double)num2s << endl;

      cout << s << "discrete_bool_ratio:" << ((double)
v.discrete.size())/(double)v.BOOLs <<endl;
      ...
      cout << s << "auxvar_branching_ratio:" <<
((double)auxvars)/(double)branchingvars <<endl;
      ...
      const SysInt
totalarity=checked_cast<SysInt>(std::accumulate(arities.begin(), arities.end(),
(DomainInt)0));

      cout << s << "TotalArity: " << totalarity << endl;
      cout << s << "arity_mean:" << ((double)totalarity)/(double) arities.size()
<< endl;
      cout << s << "arity_mean_normalised:" << (((double)totalarity)/(double)
arities.size())/((double) varcount) << endl;
      cout << s << "cts_per_var_mean:" << ((double)totalarity)/(double) varcount
<< endl;
...
(((double)totalarity)/((double) varcount))/((double) c.size()) << endl;
      ...
      SysInt reify=0, lex=0, unary=0, nullary=0, element=0, minmax=0,
occurrence=0;
      for(list<ConstraintBlob>::iterator i=c.begin(); i!=c.end(); ++i)
      {
          classifyConstraint(*i, &alldiff, &sums, &or_atleastk, &ternary,
&binary, &table, &reify, &lex, &unary, &nullary, &element, &minmax, &occurrence,
&alldiffdomovervars, v);
      }
      ...
      cout << s << "alldiffdomovervars_25:" <<
alldiffdomovervars[alldiffdomovervars.size()/4] <<endl;
      cout << s << "alldiffdomovervars_50:" <<
alldiffdomovervars[alldiffdomovervars.size()/2] <<endl;
      cout << s << "alldiffdomovervars_75:" <<
alldiffdomovervars[(alldiffdomovervars.size()*3)/4] <<endl;
      cout << s << "alldiffdomovervars_100:" << alldiffdomovervars.back()
<<endl;
...
(alldiffdomovervarstotal)/(double) alldiffdomovervars.size() << endl;
      ...
      cout << s << "alldiff_proportion:" << ((double)alldiff)/(double)c.size()
<< endl;
      cout << s << "sums_count:" << sums << endl;
      cout << s << "sums_proportion:" << ((double)sums)/(double)c.size() <<
endl;
      cout << s << "or_atleastk_count:" << or_atleastk << endl;
      cout << s << "or_atleastk_proportion:" <<
((double)or_atleastk)/(double)c.size() << endl;
      cout << s << "ternary_count:" << ternary << endl;
      cout << s << "ternary_proportion:" << ((double)ternary)/(double)c.size()
<< endl;
      cout << s << "binary_count:" << binary << endl;
      cout << s << "binary_proportion:" << ((double)binary)/(double)c.size() <<
endl;
      cout << s << "reify_count:" << reify << endl;
      cout << s << "reify_proportion:" << ((double)reify)/(double)c.size() <<
endl;
      cout << s << "table_count:" << table << endl;
      cout << s << "table_proportion:" << ((double)table)/(double)c.size() <<
endl;
      cout << s << "lex_count:" << lex << endl;
      cout << s << "lex_proportion:" << ((double)lex)/(double)c.size() << endl;
      cout << s << "unary_count:" << unary << endl;
      cout << s << "unary_proportion:" << ((double)unary)/(double)c.size() <<
endl;
      cout << s << "nullary_count:" << nullary << endl;
      cout << s << "nullary_proportion:" << ((double)nullary)/(double)c.size()
<< endl;
      cout << s << "element_count:" << element << endl;
      cout << s << "element_proportion:" << ((double)element)/(double)c.size()
<< endl;
      cout << s << "minmax_count:" << minmax << endl;
      cout << s << "minmax_proportion:" << ((double)minmax)/(double)c.size() <<
endl;
      cout << s << "occurrence_count:" << occurrence << endl;
      cout << s << "occurrence_proportion:" <<
((double)occurrence)/(double)c.size() << endl;
...
((var_sets[i].back() < var_sets[j].front()) || (var_sets[j].back() <
var_sets[i].front()))))
                set_intersection(var_sets[i].begin(), var_sets[i].end(),
var_sets[j].begin(), var_sets[j].end(), back_inserter(inter));
              if(inter.size()>=2)
              {
                  count_2_overlaps++;
              }
      ...
      for(SysInt i = 0; i < (SysInt)var_sets.size(); ++i)
      {
          ...
          for(SysInt j = 0; j < (SysInt)size; ++j)
          {
              for(SysInt k=j+1; k<size; ++k)
              {
                  Var t1=var_sets[i][j];
                  Var t2=var_sets[i][k];
                  std::set<pair<Var, Var> >::const_iterator it=
seen_pairs.find(make_pair(t1, t2));
...
((double)count_pairs)/(((double)(varcount*(varcount-1)))/2.0) << endl;
      ...
      cout << s << "Local_Variance:" <<
partition_graph(graph.g.build_graph_info(csp, false)) << endl;
  ...
  void output_stats_tightness(vector<AbstractConstraint*> cons)
  {
      ...
      const SysInt
totaltightness=checked_cast<SysInt>(std::accumulate(tightness.begin(),
tightness.end(), (DomainInt)0));
      cout << s << "tightness_mean:" << ((double)totaltightness)/(double)
tightness.size() << endl;

      //now literal tightness
      map<pair<Var,DomainInt>,vector<DomainInt> > scores_for_varval; //all
available tightnesses for varvals
...
varvals involved in con
      for(SysInt con = 0; con < (SysInt)cons.size(); con++)
      {
        ...
        for(size_t var = 0; var < all_vars.size(); var++) {
          ...
          for(DomainInt val = all_vars[var].getInitialMin(); val <=
all_vars[var].getInitialMax(); val++) {
            scores_for_varval[make_pair(vv,
val)].push_back(cons[con]->getTightnessEstimateVarVal(var, val));
          }
        }
      }
      //now average the tightnesses
      vector<double> lit_tightness;
      for(map<pair<Var,DomainInt>,vector<DomainInt> >::iterator curr =
scores_for_varval.begin();
          curr != scores_for_varval.end();
          curr++) {
        vector<DomainInt> nums = curr->second;
        lit_tightness.push_back((double)checked_cast<SysInt>(std::accumulate(nums.begin(),
nums.end(), (DomainInt)0)) / (double)nums.size()); //mean literal tightness
      }

      std::sort(lit_tightness.begin(), lit_tightness.end());

      cout << s << "literal_tightness_0:" << lit_tightness[0] <<endl;
      cout << s << "literal_tightness_25:" <<
lit_tightness[lit_tightness.size()/4] <<endl;
      cout << s << "literal_tightness_50:" <<
lit_tightness[lit_tightness.size()/2] <<endl;
      cout << s << "literal_tightness_75:" <<
lit_tightness[(lit_tightness.size()*3)/4] <<endl;
      cout << s << "literal_tightness_100:" << lit_tightness.back() <<endl;

      double new_totaltightness=std::accumulate(lit_tightness.begin(),
lit_tightness.end(), 0.0);
      double lt_mean = (double)new_totaltightness /
(double)lit_tightness.size();
