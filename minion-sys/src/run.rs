...
pub struct RunOptions {
    ...
    ///
    /// A truncated run still returns normally; compare the reported `Nodes`
    /// against the limit to tell "finished" from "gave up".
    pub node_limit: u64,
    ...
    ///
    /// Unlike `node_limit`, a run that hits the time limit comes back as
    /// `RuntimeError::Timeout` — no context, and so no statistics.
