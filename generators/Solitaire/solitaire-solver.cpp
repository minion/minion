...
int main (int argc, char * const argv[]) {
  ...
  for(int i=0;i<BOARDSIZE;++i)
  {
	if(i == start_boardpos)
	{
	  ...
	  // add_constraint(UnaryEqualCon(boards[0][i], compiletime_val<int, 0>()));
	  // add_constraint(UnaryEqualCon(boards[NOMOVES][i], compiletime_val<int, 1>()));
	}
	else
	{
	  CSP.constraint(Eq, boards[0][i], Var(Constant, 1));
	  CSP.constraint(Eq, boards[NOMOVES][i], Var(Constant, 0));
	  //add_constraint(UnaryEqualCon(boards[0][i], compiletime_val<int, 1>()));
	  //add_constraint(UnaryEqualCon(boards[NOMOVES][i], compiletime_val<int, 0>()));
	}
  }
	...
	for(int loop1 = 0; loop1 < 7; ++loop1)
	  for(int loop2 = 0; loop2 < 7; ++loop2)
	  {
		  ...
		  // This is in a {} pair just to hide local variables
		{
		  ...
		  push_back_if_valid(v,loop1,loop2+2,loop1,loop2);
		  vector<Var> w(v.size());
		  for(int loop = 0; loop < (int)v.size(); loop++)
		  { w[loop] = transition_vars[i][v[loop]]; }
		  CSP.constraintReify(zero2one[i][curBoardPos], SumGeq, w, Var(Constant, 1));
		  //	    add_constraint(rareifyCon(BoolGreaterEqualSumCon(w, compiletime_val<int, 1>()), zero2one[i][curBoardPos]));
		}
		{
			vector<Var> and_vars;
...

		  {
			...
			//add_constraint(rareifyCon(BoolGreaterEqualSumCon(w, compiletime_val<int, 1>()), one2zero[i][curBoardPos]));
