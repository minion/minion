...
without debug information are provided for Mac, Linux, and Cygwin.


\subsection{Installation instructions for Windows}

Download the windows archive minion-x.y.z-cygwin.tar.gz and unpack,
you should find Minion executable minion.exe along with the required library
cygwin1.dll. The executable should work from the windows command shell cmd.exe.


\subsection{Installation instructions for Mac}

Download the Mac archive minion-x.y.z-mac.tar.gz and unpack. The contents
include universal binary `minion' which should work on both Intel and PowerPC
...
the binary `minion'. The executable is linked statically.

\bigskip

All versions come with a debug variant as well. The debug executables are
contained in the minion-x.y.z-debug-<os>.tar.gz.
...
version between 1.33 and 1.38.0 inclusive should work. If you have installed
...
installation directory or the environment variables \verb+BOOST_INCLUDEDIR+ and
\verb+BOOST_LIBRARYDIR+ before running CMake.
